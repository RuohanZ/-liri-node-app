...
//Functions for different command
        ...
        if (!error && response.statusCode === 200) {
            for (var i = 0;i<JSON.parse(body).Ratings.length;i++){
                if(JSON.parse(body).Ratings[i].Source == "Rotten Tomatoes"){
                    var tomatoRate = JSON.parse(body).Ratings[i].Value;
                }
                if(!tomatoRate && i == JSON.parse(body).Ratings.length-1){
                    tomatoRate = "Not Available";
                }
            }
